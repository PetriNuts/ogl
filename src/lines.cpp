...
void wxLineShape::DrawRegion(wxDC& dc, wxShapeRegion *region, double x, double y)
{
  ...
  if (region->GetFormattedText().GetCount() > 0)
  {
      ...
      dc.DrawRectangle(WXROUND(xp - w/2.0), WXROUND(yp - h/2.0), WXROUND(w), WXROUND(h));
  ...
  for (int i = 0; i < 3; i ++)
  {
    ...
    if (regionNode)
    {
      ...
      if (region->m_formattedText.GetCount() > 0)
      {
        ...
        cx += xp;
        cy += yp;
        double rLeft = (cx - (cw/2.0));
        double rTop = (cy - (ch/2.0));
        double rRight = (cx + (cw/2.0));
        double rBottom = (cy + (ch/2.0));
...
void wxLineShape::DrawArrow(wxDC& dc, wxArrowHead *arrow, double xOffset, bool proportionalOffset)
{
  ...
  switch (arrow->GetPosition())
  {
    case ARROW_POSITION_START:
    {
      ...
      {
        double totalLength = 
				sqrt((second_line_point->x - first_line_point->x)*(second_line_point->x - first_line_point->x) +
    ...
    case ARROW_POSITION_END:
    {
      ...
      {
        double totalLength =
				sqrt((second_last_line_point->x - last_line_point->x)*(second_last_line_point->x - last_line_point->x) +
    ...
    case ARROW_POSITION_MIDDLE:
    {
      ...
      {
        double totalLength =
				sqrt((second_last_line_point->x - x)*(second_last_line_point->x - x) +
  ...
  if ((arrow->GetYOffset() != 0.0) && !m_ignoreArrowOffsets)
  {
     ...
     */
     ...
     double x4 = (x3 - (d*sin(theta)));
     double y4 = (y3 + (d*cos(theta)));
  ...
  switch (arrow->_GetType())
  {
    case ARROW_ARROW:
    {
      ...
      double arrowWidth = arrowLength/3.0;
      ...
      wxPoint points[4];
      points[0].x = WXROUND(tip_x); points[0].y = WXROUND(tip_y);
      points[1].x = WXROUND(side1_x); points[1].y = WXROUND(side1_y);
      points[2].x = WXROUND(side2_x); points[2].y = WXROUND(side2_y);
      points[3].x = WXROUND(tip_x); points[3].y = WXROUND(tip_y);
    ...
    case ARROW_FILLED_CIRCLE:
    {
      ...
      double x1 = (x - (diameter/2.0));
      double y1 = (y - (diameter/2.0));
      ...
      dc.DrawEllipse(WXROUND(x1), WXROUND(y1), WXROUND(diameter), WXROUND(diameter));
      break;
    }
      ...
      if (arrow->GetMetaFile())
      {
        ...
        */
        ...
        if ((x1 == x2) && (y1 == y2))
          ...
          theta = (myPi + atan((y2 - y1)/(x2 - x1)));

        else if ((x2 > x1) && (y2 < y1))
          theta = (2*myPi + atan((y2 - y1)/(x2 - x1)));
        ...
        if (m_erasing)
        {
          ...
          int extraPixels = 4;
          dc.DrawRectangle(WXROUND(deltaX + x + minX - (extraPixels/2.0)), WXROUND(deltaY + y + minY - (extraPixels/2.0)),
                           WXROUND(maxX - minX + extraPixels), WXROUND(maxY - minY + extraPixels));
        }
        else
...
void wxLineShape::OnErase(wxDC& dc)
{
    ...
    {
      dc.DrawRectangle(WXROUND(m_xpos - (bound_x/2.0) - 2.0), WXROUND(m_ypos - (bound_y/2.0) - 2.0),
                        WXROUND(bound_x+4.0),  WXROUND(bound_y+4.0));
    }
    else
    {
      m_erasing = true;
...
int wxLineShape::FindLinePosition(double x, double y)
{
  ...
  double centreDistance = sqrt((x - m_xpos)*(x - m_xpos) + (y - m_ypos)*(y - m_ypos));
  double startDistance = sqrt((x - startX)*(x - startX) + (y - startY)*(y - startY));
  double endDistance = sqrt((x - endX)*(x - endX) + (y - endY)*(y - endY));
...
void wxLabelShape::OnDraw(wxDC& dc)
{
  if (m_lineShape && !m_lineShape->GetDrawHandles())
    ...
    double x1 = (m_xpos - m_width/2.0);
    double y1 = (m_ypos - m_height/2.0);
...
bool wxLineShape::OnLabelMovePre(wxDC& dc, wxLabelShape* labelShape, double x, double y, double WXUNUSED(old_x), double WXUNUSED(old_y), bool WXUNUSED(display))
{
  ...
  // each region.
  labelShape->m_shapeRegion->SetPosition((x - xx), (y - yy));

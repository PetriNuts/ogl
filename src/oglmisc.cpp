                       ...
                       int formatMode)
{
  ...
  if (formatMode & FORMAT_CENTRE_HORIZ)
    ...
    xoffset = (m_xpos - (width / 2.0));

  if (formatMode & FORMAT_CENTRE_VERT)
    yoffset = m_ypos;
  else
    yoffset = (m_ypos - (height / 2.0));

  dc.SetClippingRegion(
                    WXROUND(m_xpos - width/2.0), WXROUND(m_ypos - height/2.0),
                    WXROUND(width+1), WXROUND(height+1)); // +1 to allow for rounding errors

...
void wxShape::OnDrawBranches(wxDC& dc, int attachment, bool erase)
{
    ...
    // Draw neck
    dc.DrawLine(WXROUND(root.x), WXROUND(root.y), WXROUND(neck.x), WXROUND(neck.y));

    if (count > 1)
    {
        // Draw shoulder-to-shoulder line
        dc.DrawLine(WXROUND(shoulder1.x), WXROUND(shoulder1.y), WXROUND(shoulder2.x), WXROUND(shoulder2.y));
    }
    ...
    for (i = 0; i < count; i++)
    {
        ...
        dc.DrawLine(WXROUND(stemPt.x), WXROUND(stemPt.y), WXROUND(pt.x), WXROUND(pt.y));

        if ((GetBranchStyle() & BRANCHING_ATTACHMENT_BLOB) && (count > 1))
        {
            long blobSize=6;
            dc.DrawEllipse(WXROUND(stemPt.x - (blobSize/2.0)), WXROUND(stemPt.y - (blobSize/2.0)), blobSize, blobSize);
        }
    }
}

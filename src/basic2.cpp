...
void wxEllipseShape::OnDraw(wxDC& dc)
{
    if (m_shadowMode != SHADOW_NONE)
    {
      if (m_shadowBrush)
        dc.SetBrush(* m_shadowBrush);
      dc.SetPen(* g_oglTransparentPen);
      dc.DrawEllipse(WXROUND((m_xpos - GetWidth()/2) + m_shadowOffsetX),
                     WXROUND((m_ypos - GetHeight()/2) + m_shadowOffsetY),
                     WXROUND(GetWidth()), WXROUND(GetHeight()));
    }
    ...
    if (m_brush)
      dc.SetBrush(* m_brush);
    dc.DrawEllipse(WXROUND(m_xpos - GetWidth()/2), WXROUND(m_ypos - GetHeight()/2), WXROUND(GetWidth()), WXROUND(GetHeight()));
}
